...
(function () {
  ...
  async function loadProducts() {
    ...
    // Cargar e incorporar cosechas registradas por agricultores locales desde localStorage
    const savedFarmerProducts = getSavedFarmerProducts();
    if (savedFarmerProducts.length > 0) {
      AppState.products = [...savedFarmerProducts, ...AppState.products];
      console.log(`[AgroPasco] ${savedFarmerProducts.length} cosechas de agricultores locales restauradas desde almacenamiento.`);
    }
  ...
  function createProductCardElement(prod) {
    ...
    card.className =
      "group bg-white rounded-3xl border border-slate-200/90 shadow-sm hover:shadow-xl hover:border-brand-300 transition-all duration-300 flex flex-col overflow-hidden";
    ...
    card.innerHTML = `
      ...
      <div class="p-5 flex-1 flex flex-col justify-between space-y-4">
        <div class="space-y-2">
          <!-- Título -->
          <h3 class="font-extrabold text-slate-900 text-base leading-snug group-hover:text-brand-700 transition-colors">
            ${prod.name}
          </h3>
          ...
          <div class="flex items-baseline justify-between">
            <div>
              ...
              <div class="flex items-baseline space-x-1">
                ...
                <span class="text-2xl font-black text-brand-700">${prod.price.toFixed(2)}</span>
    ...
    const qtyDisplay = card.querySelector(`#qty-display-${prod.id}`);
...
¡Muchas gracias por apoyar a las familias campesinas de Pasco! 🇵🇪`;
  ...
  function setupFarmerFormEvents() {
    // 1. Selector de archivo de imagen (carga desde celular o PC)
    if (DOM.farmerImageFile) {
      DOM.farmerImageFile.addEventListener("change", (e) => {
        ...
        if (file) {
          ...
          reader.onload = (evt) => {
            uploadedFarmerImageDataUrl = evt.target.result;
            if (DOM.farmerPreviewImg) DOM.farmerPreviewImg.src = uploadedFarmerImageDataUrl;
            if (DOM.farmerFileLabel) DOM.farmerFileLabel.textContent = file.name;
            if (DOM.farmerPreviewHint) DOM.farmerPreviewHint.textContent = "Foto cargada desde tu dispositivo";
   ...
   */
  function handleFarmerProductSubmit(e) {
    e.preventDefault();
    ...
    const priceVal = DOM.farmerPriceProd ? parseFloat(DOM.farmerPriceProd.value) : 0;
    ...
    const phone = DOM.farmerPhoneProd ? DOM.farmerPhoneProd.value.trim() : APP_CONFIG.centralWhatsAppNumber;
    const desc = DOM.farmerDescProd ? DOM.farmerDescProd.value.trim() : "";

    // Validaciones estrictas
    if (!name || name.length < 3) {
      showToast("Nombre requerido", "Ingresa el nombre del producto o cosecha (mínimo 3 letras).", "warning");
    ...
    if (isNaN(priceVal) || priceVal <= 0) {
      showToast("Precio inválido", "El precio por unidad debe ser un número mayor a 0 Soles.", "warning");
      if (DOM.farmerPriceProd) DOM.farmerPriceProd.focus();
      return;
    }

    if (!phone || phone.length < 8) {
      showToast("Teléfono requerido", "Ingresa un número de celular o WhatsApp válido para recibir pedidos.", "warning");
      if (DOM.farmerPhoneProd) DOM.farmerPhoneProd.focus();
      return;
    }
    ...
    // 2. Guardar en localStorage para que persista al recargar
    const customList = getSavedFarmerProducts();
    customList.unshift(newProduct);
    saveFarmerProductsToStorage(customList);

    // 3. Inicializar selector de cantidad para la tarjeta
    AppState.catalogQuantities[newProduct.id] = 1;

    // 4. Restablecer categoría a "all" para asegurar que sea visible de inmediato
    ...
    // 5. Renderizar catálogo y pastillas de categorías inmediatamente
    renderCategoryPills();
    renderProductsCatalog();
    updateResetFiltersButton();
    ...
    showToast(
      ...
      `"${newProduct.name}" de ${newProduct.producer} ya aparece en la página principal.`,
      "success",
      5500
    );

    // 8. Desplazar suavemente al catálogo
