...
(function () {
  ...
  async function loadProducts() {
    ...
    if (savedFarmerProducts.length > 0) {
      const normalizedFarmerProducts = savedFarmerProducts.map((p) => ({
        ...p,
        price: typeof p.price === "number" ? p.price : (parseFloat(p.price) || 0),
        isLocalUpload: true
      }));
      AppState.products = [...normalizedFarmerProducts, ...AppState.products];
      console.log(`[AgroPasco] ${normalizedFarmerProducts.length} cosechas de agricultores locales restauradas desde almacenamiento.`);
    }
...
¡Muchas gracias por apoyar a las familias campesinas de Pasco! 🇵🇪`;
  ...
  async function handleFarmerProductSubmit(e) {
    ...
    // 3. Inicializar selector de cantidad para la nueva tarjeta
