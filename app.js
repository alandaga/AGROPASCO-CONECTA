...
(function () {
  ...
  /* ============================================================================
     4.2. INTEGRACIÓN CON FIREBASE CLOUD FIRESTORE (TIEMPO REAL GLOBAL EN LA NUBE)
     ============================================================================ */
  ...
  /**
   * Verifica si las credenciales de Firebase han sido configuradas en config.js.
   * @returns {boolean}
   */
  function isFirebaseConfigured() {
    return !!(
      typeof firebase !== "undefined" &&
      APP_CONFIG.enableFirebase &&
      APP_CONFIG.firebaseConfig &&
      APP_CONFIG.firebaseConfig.apiKey &&
      APP_CONFIG.firebaseConfig.apiKey !== "PEGAR_AQUI_TU_API_KEY" &&
      !APP_CONFIG.firebaseConfig.apiKey.includes("PEGAR_AQUI") &&
      APP_CONFIG.firebaseConfig.projectId &&
      APP_CONFIG.firebaseConfig.projectId !== "tu-proyecto-id" &&
      !APP_CONFIG.firebaseConfig.projectId.includes("tu-proyecto")
    );
  }

  /**
   * Inicializa la conexión oficial con Google Cloud Firestore.
   * @returns {boolean}
   */
  function initFirestore() {
    if (!isFirebaseConfigured()) {
      console.log("[AgroPasco] Firebase Cloud Firestore disponible: pega tus credenciales en config.js para activar la base de datos en la nube global.");
      return false;
    }
    try {
      ...
      console.log(`[AgroPasco] ¡Conexión exitosa a Google Cloud Firestore! (Proyecto: ${APP_CONFIG.firebaseConfig.projectId})`);
      return true;
    } catch (err) {
      console.warn("[AgroPasco] Error al inicializar Firebase Firestore:", err);
      firestoreDb = null;
      return false;
    }
  }
  ...
  /**
   * Suscribe la aplicación a la colección de Firestore en tiempo real usando onSnapshot.
   * Cualquier producto creado, editado o eliminado en cualquier parte del mundo
   * se actualizará automáticamente en la pantalla de todos los usuarios sin recargar.
   */
  function subscribeToFirestoreRealtime() {
    if (!firestoreDb) return;

    try {
      const collectionName = APP_CONFIG.firestoreCollection || "productos_cosechas";
      ...
      firestoreUnsubscribe = collectionRef.onSnapshot(
        (snapshot) => {
          ...
          // Ordenar las cosechas más recientes primero
          firestoreProducts.sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0));

          console.log(`[AgroPasco Firestore] Recibidas ${firestoreProducts.length} cosechas en vivo desde Cloud Firestore.`);

          // Si otro usuario acaba de publicar una cosecha, mostrar toast
          if (!isInitialFirestoreLoad) {
            snapshot.docChanges().forEach((change) => {
              if (change.type === "added") {
                const addedData = change.doc.data();
                showToast(
                  "¡Nueva Cosecha en Vivo!",
                  `"${addedData.name}" de ${addedData.producer} acaba de ser publicada en la nube.`,
                  "info",
                  5500
                );
              }
          ...
          saveFarmerProductsToStorage(firestoreProducts);

          // Integrar con productos base
          const baseProducts = AppState.products.filter((p) => !p.isLocalUpload);
          AppState.products = [...firestoreProducts, ...baseProducts];
          ...
          // Indicar origen de datos
          AppState.dataSource = "firestore";
          updateDataSourceIndicator();

          // Renderizar catálogo actualizado en la página principal
          renderCategoryPills();
          renderProductsCatalog();
        },
        (error) => {
          console.warn("[AgroPasco Firestore] Error en la suscripción en tiempo real:", error);
        }
      );
    } catch (err) {
      console.warn("[AgroPasco Firestore] No se pudo iniciar el listener onSnapshot:", err);
    }
  }
  ...
  async function loadProducts() {
    ...
    // Cargar e incorporar cosechas: Firebase Cloud Firestore > Servidor Central > LocalStorage
    const firestoreReady = initFirestore();
    if (firestoreReady) {
      // Iniciar suscripción en tiempo real de Firebase Firestore
      subscribeToFirestoreRealtime();
    } else {
      // Si Firebase no está configurado con credenciales, usar el servidor central o almacenamiento local
      ...
      if (remoteProducts !== null) {
        ...
        console.log(`[AgroPasco] ${sharedFarmerList.length} cosechas compartidas cargadas desde el servidor central.`);
      } else {
        sharedFarmerList = getSavedFarmerProducts();
        console.log(`[AgroPasco] Servidor central no detectado, usando almacenamiento local (${sharedFarmerList.length} cosechas).`);
      }
  ...
  function updateDataSourceIndicator() {
    ...
    if (AppState.dataSource === "firestore") {
      DOM.dataSourceText.textContent = "Cloud Firestore en vivo ☁️";
      DOM.dataSourceStatus.classList.remove("bg-slate-100", "text-slate-600", "bg-emerald-50", "text-emerald-700", "border-emerald-200");
      DOM.dataSourceStatus.classList.add("bg-amber-50", "text-amber-800", "border-amber-300");
    } else if (AppState.dataSource === "sheets") {
      DOM.dataSourceText.textContent = "Google Sheets en vivo";
      DOM.dataSourceStatus.classList.remove("bg-slate-100", "text-slate-600", "bg-amber-50", "text-amber-800", "border-amber-300");
      ...
      DOM.dataSourceStatus.classList.remove("bg-emerald-50", "text-emerald-700", "border-emerald-200", "bg-amber-50", "text-amber-800", "border-amber-300");
    ...
    if (btnDelete) {
      btnDelete.addEventListener("click", async (e) => {
        ...
        if (confirm(`¿Deseas retirar tu publicación de "${prod.name}" del catálogo?`)) {
          ...
          // 1. Eliminar de Cloud Firestore si está conectado
          if (firestoreDb) {
            const colName = APP_CONFIG.firestoreCollection || "productos_cosechas";
            firestoreDb.collection(colName).doc(prod.id).delete().catch((err) => {
              console.warn("[AgroPasco Firestore] Error al eliminar de Cloud Firestore:", err);
            });
          }

          // 2. Eliminar del servidor central local
...
¡Muchas gracias por apoyar a las familias campesinas de Pasco! 🇵🇪`;
  ...
  async function handleFarmerProductSubmit(e) {
    ...
    // 1. Enviar a Cloud Firestore en la nube (Persistencia global en tiempo real)
    if (firestoreDb) {
      const colName = APP_CONFIG.firestoreCollection || "productos_cosechas";
      firestoreDb
        .collection(colName)
        ...
        .set(newProduct)
        .then(() => {
          console.log(`[AgroPasco Firestore] Cosecha '${newProduct.name}' guardada globalmente en Cloud Firestore.`);
        })
        .catch((err) => {
          console.warn("[AgroPasco Firestore] Error al guardar en Cloud Firestore:", err);
        });
    }

    // 2. Anteponer al catálogo activo en memoria para respuesta visual instantánea
    AppState.products = [newProduct, ...AppState.products.filter((p) => p.id !== newProduct.id)];

    // 3. Guardar en localStorage para respaldo local
    const customList = getSavedFarmerProducts().filter((p) => p.id !== newProduct.id);
    customList.unshift(newProduct);
    saveFarmerProductsToStorage(customList);

    // 4. Enviar al servidor central local como respaldo multiusuario
    apiSaveFarmerProduct(newProduct);
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    updateCartUI();
    loadProducts().then(() => {
      // Si Firestore no está conectado, iniciar sondeo en vivo del servidor central
      if (!firestoreDb) {
        startLiveSync();
      }
    });
